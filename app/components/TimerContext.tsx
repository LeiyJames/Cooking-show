...
interface TimerContextType {
  ...
  updateInputValue: (
    field: "inputMinutes" | "inputSeconds",
    value: string,
    dishName: string,
  ) => void;
...
export function TimerProvider({ children }: { children: React.ReactNode }) {
    ...
    if (savedTimers) {
      try {
        ...
        if (hasCorruptedData) {
          ...
          const normalizedTimers = Object.keys(timersData).reduce(
            (acc, dishName) => {
              acc[dishName] = {
                ...timersData[dishName],
  ...
  const saveTimerStates = useCallback(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      const timersToSave = Object.keys(timers).reduce(
        (acc, dishName) => {
          acc[dishName] = {
            ...timers[dishName],
            timestamp: Date.now(),
  ...
  useEffect(() => {
    ...
    const runningDishes = Object.entries(timers).filter(
      ([_, timer]) => timer.isRunning && timer.timeLeft > 0,
    );
  ...
  const getTimerState = useCallback(
    (dishName: string): TimerState => {
      ...
      if (typeof state.inputMinutes !== "string" || state.inputMinutes === "")
        state.inputMinutes = "0";
      if (typeof state.inputSeconds !== "string" || state.inputSeconds === "")
  ...
  const startTimer = useCallback(
    (minutes: number, seconds: number, dishName: string) => {
  ...
  const setRecommendedTime = useCallback(
    (minutes: number, dishName: string) => {
      setTimers((prev) => ({
        ...prev,
        [dishName]: {
          ...prev[dishName],
  ...
  const getCurrentRunningDish = useCallback(() => {
    const runningDish = Object.entries(timers).find(
