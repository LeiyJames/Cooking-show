...
export default function FilipinoRecipeSections({
  dish,
}: FilipinoRecipeSectionsProps) {
  ...
  const notesRef = useRef(dish.defaultNotes);
  ...
  useEffect(() => {
    ...
    const savedIngredients = localStorage.getItem(
      `cookingIngredients_${dish.title}`,
    );
  ...
  // Cleanup timeout and flush pending saves on unmount
  useEffect(() => {
    return () => {
      ...
      content: (
        <ul className="space-y-2">
          {ingredients.map((ingredient, index) => (
            ...
            >
              <span className="text-cooking-500 dark:text-cooking-400 mt-1 transition-colors duration-300">
                •
              </span>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-cooking-600 dark:text-cooking-400 transition-colors duration-300">
                {section.icon}
              </span>
