...
export default function FilipinoRecipeSections({ dish }: FilipinoRecipeSectionsProps) {
  ...
  const saveNotes = useCallback((value: string, title: string) => {
  ...
  useEffect(() => {
    return () => {
      // Synchronously flush the latest notes to localStorage on unmount to prevent data loss
      if (saveTimeoutRef.current) {
        clearTimeout(saveTimeoutRef.current)
